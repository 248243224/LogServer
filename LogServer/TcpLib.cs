...
namespace TcpLib
{
    ...
    public abstract class TcpServiceProvider : ICloneable
    {
        ...
        /// </SUMMARY>
        public abstract void OnDropConnection(string connectionId);
    }



    public class TcpServer : LogServer.ITcpServer
    {
        private string _ip;
        private int _port;
        ...
        private void ReceivedDataReady_Handler(IAsyncResult ar)
        {
            ...
            if (!st.Connected)
                return;
            st._conn.EndReceive(ar, out errorCode);

            //Im considering the following condition as a signal that the
            //remote host droped the connection.
            if (errorCode != SocketError.Success || st._conn.Available == 0) DropConnection(st);
            else
            {
        ...
        internal void DropConnection(ConnectionState st)
        {
            lock (this)
            {
                string clientId = st.RemoteEndPoint.ToString();
                st._conn.Shutdown(SocketShutdown.Both);
                st._conn.Close();
                if (_connections.Contains(st))
                    _connections.Remove(st);
                st._provider.OnDropConnection(clientId);
            }
        }
